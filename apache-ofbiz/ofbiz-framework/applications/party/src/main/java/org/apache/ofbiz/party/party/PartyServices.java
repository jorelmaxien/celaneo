...
public class PartyServices {
    ...
    public static final String SAMPLE_XLS_FILE_PATH = "./spreadsheet/EmployesConakry.xlsx";
    //public static final String SPLE_XLS_FILE_PATH = "./spreadsheet/fonctionParService.xls";
...
//        String registrationNumber = (String) context.get("registrationNumber");
        String registrationNumber = (String) context.get("registrationNumber");
        ...
        if (party != null) {
            ...
            Debug.logWarning(registrationNumber, module);
        ...
        System.out.println("Context birth Date "+ context.get("birthDate"));
    ...
    public static Map<String, Object> findParty(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        // get the positions
        try {
            List<GenericValue> positions = EntityQuery.use(delegator).from("EmplPosition").queryList();
            result.put("positions", positions);
        } catch (GenericEntityException e) {
            String errMsg = "Error looking up positions: " + e.toString();
            Debug.logError(e, errMsg, module);
            return ServiceUtil.returnError(UtilProperties.getMessage(resource,
                    "PartyLookupRoleTypeError",
                    UtilMisc.toMap("errMessage", e.toString()), locale));
        ...
        try {
            emplPositionId = (String) context.get("emplPositionId");
            if (UtilValidate.isNotEmpty(emplPositionId)) {
                GenericValue currentPosition = EntityQuery.use(delegator).from("EmplPosition").orderBy("partyId").where("emplPositionId", emplPositionId).cache().queryOne();
                result.put("currentPosition", currentPosition);

                System.out.println(currentPosition);
            }
        } catch (GenericEntityException e) {
            String errMsg = "Error looking up current emplPosition: " + e.toString();
            Debug.logError(e, errMsg, module);
            return ServiceUtil.returnError(UtilProperties.getMessage(resource,
                    "PartyLookupRoleTypeError",
                    UtilMisc.toMap("errMessage", e.toString()), locale));
        ...
        // get the services
        try {
            List<GenericValue> services = EntityQuery.use(delegator).from("Party").queryList();
            result.put("services", services);
        } catch (GenericEntityException e) {
            String errMsg = "Error looking up parties: " + e.toString();
            Debug.logError(e, errMsg, module);
            return ServiceUtil.returnError(UtilProperties.getMessage(resource,
                    "PartyLookupRoleTypeError",
                    UtilMisc.toMap("errMessage", e.toString()), locale));
        ...
        String serviceId;
        try {
            serviceId = (String) context.get("serviceId");
            if (UtilValidate.isNotEmpty(serviceId)) {
                GenericValue currentService = EntityQuery.use(delegator).from("Party").orderBy("partyId").where("emplPositionId", emplPositionId).cache().queryOne();
                result.put("currentService", currentService);
            }
        } catch (GenericEntityException e) {
            String errMsg = "Error looking up current Party: " + e.toString();
            Debug.logError(e, errMsg, module);
            return ServiceUtil.returnError(UtilProperties.getMessage(resource,
                    "PartyLookupRoleTypeError",
                    UtilMisc.toMap("errMessage", e.toString()), locale));
        ...
        if ("Y".equals(lookupFlag)) {
            ...
            dynamicView.addAlias("PT", "registrationNumber");
            ...
            fieldsToSelect.add("registrationNumber");
            ...
            if (!"Y".equals(showAll)) {
                ...
                if (UtilValidate.isNotEmpty(registrationNumber)) {
                    paramList = paramList + "&registrationNumber=" + registrationNumber;
                    andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("registrationNumber"), EntityOperator.LIKE, EntityFunction.UPPER("%" + registrationNumber + "%")));
                }
                ...
                // filter on Position member
                if (emplPositionId != null && !"ANY".equals(emplPositionId)) {
                    paramList = paramList + "&emplPositionId=" + emplPositionId;

                    // add position view
                    dynamicView.addMemberEntity("EP", "EmplPositionFulfillment");
                    dynamicView.addAlias("EP", "emplPositionId");
                    dynamicView.addViewLink("PT", "EP", Boolean.FALSE, ModelKeyMap.makeKeyMapList("partyId"));

                    // add the expr
                    andExprs.add(EntityCondition.makeCondition("emplPositionId", EntityOperator.EQUALS, emplPositionId));
                ...
                // filter on Service member
                if (serviceId != null && !"ANY".equals(serviceId)) {
                    paramList = paramList + "&serviceId=" + serviceId;

                    // add position view
                    dynamicView.addMemberEntity("EPM", "Employment");
                    dynamicView.addAlias("EPM", "partyIdTo");
                    dynamicView.addViewLink("PT", "EPM", Boolean.FALSE, ModelKeyMap.makeKeyMapList("partyId"));

                    // add the expr
                    andExprs.add(EntityCondition.makeCondition("serviceId", EntityOperator.EQUALS, serviceId));

                    fieldsToSelect.add("serviceId");
                }

                // filter on Position
                if (UtilValidate.isNotEmpty(emplPositionId)) {
                    paramList = paramList + "&emplPositionId=" + emplPositionId;
                    andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("emplPositionId"), EntityOperator.LIKE, EntityFunction.UPPER("%"+emplPositionId+"%")));
                }
                // filter on Service
                if (UtilValidate.isNotEmpty(serviceId)) {
                    paramList = paramList + "&serviceId=" + serviceId;
                    andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("serviceId"), EntityOperator.LIKE, EntityFunction.UPPER("%"+serviceId+"%")));
                }
                // ----
    ...
    public static Map<String, Object> importParty1(DispatchContext dctx, Map<String, Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        Locale locale = (Locale) context.get("locale");
        GenericValue userLogin = (GenericValue) context.get("userLogin");
    ...
    public static Map<String, Object> importParty(DispatchContext dctx, Map<String, Object> context)throws Exception {
        ...
        int numero=0;
        int num=0;
        String partyEmp = "empl";
        String partyDep = "dep";
        GenericValue ser = null;
        GenericValue hSer = null;
        Map<String, Object> emplpoH = null;
        List <GenericValue> emplpo = null;
        GenericValue hFuncT = null;
        GenericValue funcT = null;
        GenericValue pers = null;
        String mat=null;



       // File file = (File) context.get("uploadedFile");

            try (Workbook workbook = WorkbookFactory.create(new File(SAMPLE_XLS_FILE_PATH))) {
                Sheet sheet = workbook.getSheetAt(1);
                DataFormatter dataFormatter = new DataFormatter();
                Iterator<Row> rowIterator = sheet.rowIterator();
                while (rowIterator.hasNext()) {
                    Row row = rowIterator.next();
                    mat=dataFormatter.formatCellValue(row.getCell(0));
                    pers=EntityQuery.use(delegator).from("Party").where("registrationNumber", mat).cache().queryOne();
                    String partyId = partyEmp.concat(Integer.toString(numero));
                    String birthDate = dataFormatter.formatCellValue(row.getCell(11));
                    if (numero!=0 && pers==null && mat.length()!=0) {
                        Map<String, Object> person = UtilMisc.toMap(
                                "partyId", partyId,
                                "firstName", dataFormatter.formatCellValue(row.getCell(2)),
                                "lastName", dataFormatter.formatCellValue(row.getCell(3)),
                                "preferredCurrencyUomId", "GNF",
                                "salutation", dataFormatter.formatCellValue(row.getCell(1)),
                                "familyAllowances", dataFormatter.formatCellValue(row.getCell(20)),
                                "gender", dataFormatter.formatCellValue(row.getCell(13)),
                                "registrationNumber", mat,
                                "socialSecurityNumber", dataFormatter.formatCellValue(row.getCell(28)),
                                "birthDate",   new java.sql.Date(DateParserUtils.parseDate(birthDate).getTime()),
                                "placeOfBirth", dataFormatter.formatCellValue(row.getCell(12)),
                                "personalTitle", dataFormatter.formatCellValue(row.getCell(23)),
                                "ethnicGroup", dataFormatter.formatCellValue(row.getCell(15)),
                                "numberOfChildren", dataFormatter.formatCellValue(row.getCell(25)),
                                "holidayMonth", dataFormatter.formatCellValue(row.getCell(22)),
                                "maritalStatus", dataFormatter.formatCellValue(row.getCell(14)),
                                "statusId", "PARTY_ENABLED",
                                "userLogin", userLogin
                        );
                        ...
                        result = dispatcher.runSync("createPartyRole", partyRole);
                        ...
                        postalAddress.put("address1", dataFormatter.formatCellValue(row.getCell(17)));
                        postalAddress.put("city", dataFormatter.formatCellValue(row.getCell(27)));
                        postalAddress.put("postalCode", "Soguipah");
                        ...
                        Map<String, Object> serviceResult = dispatcher.runSync("createPartyContactMech", UtilMisc.toMap("partyId", partyId, "contactMechId", newContactMechId, "contactMechTypeId", "POSTAL_ADDRESS", "contactMechPurposeTypeId", "GENERAL_LOCATION", "userLogin", userLogin));
                        if (ServiceUtil.isError(serviceResult)) {
                            return ServiceUtil.returnError(ServiceUtil.getErrorMessage(serviceResult));
                        }
                        ...
                        telecomNumber.put("countryCode", "224");
                        telecomNumber.put("contactNumber", dataFormatter.formatCellValue(row.getCell(18)));
                        result = dispatcher.runSync("createTelecomNumber", telecomNumber);
                        if (ServiceUtil.isError(result)) {
                            return ServiceUtil.returnError(ServiceUtil.getErrorMessage(result));
                        }
                        ...
                        serviceResult = dispatcher.runSync("createPartyContactMech", UtilMisc.toMap("partyId", partyId, "contactMechId", newContactMechId, "contactMechTypeId", "TELECOM_NUMBER",  "contactMechPurposeTypeId", "PHONE_WORK", "userLogin", userLogin));
                        if (ServiceUtil.isError(serviceResult)) {
                            return ServiceUtil.returnError(ServiceUtil.getErrorMessage(serviceResult));
                        }
                        ...
                        String compId = partyDep.concat(Integer.toString(numero));
                        ...
                        /* String val1=dataFormatter.formatCellValue(row.getCell(19));
                        if (val1==null){
                            val1=val;
                        }
                        if (val==null){
                        ...
                        hSer = EntityQuery.use(delegator).from("PartyGroup").where("groupName", val1).cache().queryOne();

                        if (hSer != null) {
                            Map<String, Object> hiringService = UtilMisc.toMap(
                                    "partyIdTo", partyId,
                                    "partyIdFrom", hSer.get("partyId"),
                                    "roleTypeIdFrom", "INTERNAL_ORGANIZATIO",
                                    "roleTypeIdTo", "EMPLOYEE",
                                    "FromDate", new java.sql.Date(DateParserUtils.parseDate(dataFormatter.formatCellValue(row.getCell(9))).getTime()),
                                    "typeContract", dataFormatter.formatCellValue(row.getCell(10)),
                                    "userLogin", userLogin
                            );
                            result = dispatcher.runSync("createEmployment", hiringService);
                            if (ServiceUtil.isError(result)) {
                                return ServiceUtil.returnError(ServiceUtil.getErrorMessage(result));
                            }
                        }*/
                        ...
                        // Now Service
                        String val=dataFormatter.formatCellValue(row.getCell(6));
                        ser = EntityQuery.use(delegator).from("PartyGroup").where("groupName", val).cache().queryOne();
                        if (ser != null) {
                            Map<String, Object> service = UtilMisc.toMap(
                                    "partyIdTo", partyId,
                                    "partyIdFrom", ser.get("partyId"),
                                    "roleTypeIdFrom", "INTERNAL_ORGANIZATIO",
                                    "roleTypeIdTo", "EMPLOYEE",
                                    "fromDate", now,
                                    "typeContract", dataFormatter.formatCellValue(row.getCell(10)),
                            ...
                            result = dispatcher.runSync("createEmployment", service);
                            if (ServiceUtil.isError(result)) {
                                return ServiceUtil.returnError(ServiceUtil.getErrorMessage(result));
                            }
                        }
                        ...
                        // Hiring function
                     /*   String fonct=dataFormatter.formatCellValue(row.getCell(8));
                        if (fonct==null){
                            fonct=dataFormatter.formatCellValue(row.getCell(7));
                        }



                        hFuncT = EntityQuery.use(delegator).from("EmplPositionType").where("description", fonct).cache().queryOne();
//
                        if(hFuncT!=null) {
                            emplpo = EntityQuery.use(delegator).from("EmplPosition").where("emplPositionTypeId", hFuncT.get("emplPositionTypeId")).cache().queryList();
                            String abc = null;
                            for (Map<String, Object> sheet1 : emplpo) {
                                if (sheet1.get("partyId").equals(hSer.get("partyId")))
                                    abc = (String) result.get("emplPositionId");
                            }

                            if (abc == null) {
                                Map<String, Object> emplpo1 = UtilMisc.toMap(
                                        "partyId", hSer.get("partyId"),
                                        "emplPositionTypeId", hFuncT.get("emplPositionTypeId"),
                                        "statusId", "EMPL_POS_ACTIVE",
                                        "userLogin", userLogin
                                );

                                result = dispatcher.runSync("createEmplPosition", emplpo1);
                                if (ServiceUtil.isError(result)) {
                                    return ServiceUtil.returnError(ServiceUtil.getErrorMessage(result));
                                }
                                abc = (String) result.get("emplPositionId");
                            ...
                            Map<String, Object> emPoFul = UtilMisc.toMap(
                                    "emplPositionId", abc,
                                    "partyId", partyId,
                                    "category", dataFormatter.formatCellValue(row.getCell(5)),
                                    "statusCompany", dataFormatter.formatCellValue(row.getCell(26)),
                                    "fromDate", new java.sql.Date(DateParserUtils.parseDate(dataFormatter.formatCellValue(row.getCell(9))).getTime()),
                                    "userLogin", userLogin
                            );

                            result = dispatcher.runSync("createEmplPositionFulfillment", emPoFul);
                            if (ServiceUtil.isError(result)) {
                                return ServiceUtil.returnError(ServiceUtil.getErrorMessage(result));
                            }
                        }*/
                        ...
                        String abcd=null;
                        String typf=null;
                        String fonct=dataFormatter.formatCellValue(row.getCell(7));
                        if (fonct!=null){
                            funcT = EntityQuery.use(delegator).from("EmplPositionType").where("description", dataFormatter.formatCellValue(row.getCell(7))).cache().queryOne();

                        if (funcT==null){
                            Map<String, Object> empPosType = UtilMisc.toMap(
                                    "description", fonct,
                                    "userLogin", userLogin
                            );

                            result = dispatcher.runSync("createEmplPositionType", empPosType);
                            if (ServiceUtil.isError(result)) {
                                return ServiceUtil.returnError(ServiceUtil.getErrorMessage(result));
                            }
                            typf=(String)result.get("emplPositionTypeId");

                        }else {
                            typf=(String)funcT.get("emplPositionTypeId");
                        emplpo=EntityQuery.use(delegator).from("EmplPosition").where("emplPositionTypeId",typf).cache().queryList();
                        for(Map<String,Object> sheet1: emplpo) {
                            if (sheet1.get("partyId").equals(ser.get("partyId")))
                                abcd=(String) result.get("emplPositionId");
                          }
                        }

                        if (abcd==null){
                            Map<String, Object> emplpo1 = UtilMisc.toMap(
                                    "partyId", ser.get("partyId"),
                                    "emplPositionTypeId", typf,
                                    "statusId", "EMPL_POS_ACTIVE",
                                    "userLogin", userLogin
                            );
                        ...
                        Map<String, Object>   emPoFul = UtilMisc.toMap(
                                "emplPositionId", abcd,
                                "partyId", partyId,
                                "category", dataFormatter.formatCellValue(row.getCell(4)),
                                "statusCompany", dataFormatter.formatCellValue(row.getCell(26)),
                                "fromDate", now,
                                "userLogin", userLogin
                        );
                        ...
                        // Qualification - niveau detudes
                        String description=dataFormatter.formatCellValue(row.getCell(16));
                        GenericValue qualType = null;
                        if(description!=null){
                            qualType = EntityQuery.use(delegator).from("PartyQualType").where("description", description).cache().queryOne();
                        }

                        if (qualType!=null){
                            Map<String, Object> partyQual= UtilMisc.toMap(
                                    "partyQualTypeId", qualType.get("partyQualTypeId"),
                                    "description",dataFormatter.formatCellValue(row.getCell(16)),
                                    "partyId", partyId,
                                    "userLogin", userLogin
                            );
                            result = dispatcher.runSync("createPartyQual", partyQual);
            ...
            }catch (GenericServiceException | GenericEntityException | IOException ex) {
         ...
         result = null;
         newPartyId = null;
         newCompanyPartyId = null;
         newContactMechId = null;
        ...
        try (Workbook workbook = WorkbookFactory.create(new File(SAMPLE_XLS_FILE_PATH))) {
            ...
            while (rowIterator.hasNext()) {
                ...
                String partyId = partyEmp.concat(Integer.toString(numero));
                String birthDate = dataFormatter.formatCellValue(row.getCell(11));
                if (num!=0 && pers==null && mat.length()!=0) {
                    Map<String, Object> person = UtilMisc.toMap(
                            "partyId", partyId,
                            "firstName", dataFormatter.formatCellValue(row.getCell(2)),
                            "lastName", dataFormatter.formatCellValue(row.getCell(3)),
                            ...
                            "gender", dataFormatter.formatCellValue(row.getCell(13)),
                            "registrationNumber", mat,
                            "socialSecurityNumber", dataFormatter.formatCellValue(row.getCell(29)),
                            "birthDate",   new java.sql.Date(DateParserUtils.parseDate(birthDate).getTime()),
                            "placeOfBirth", dataFormatter.formatCellValue(row.getCell(12)),
                            "personalTitle", dataFormatter.formatCellValue(row.getCell(24)),
                            "ethnicGroup", dataFormatter.formatCellValue(row.getCell(15)),
                            "numberOfChildren", dataFormatter.formatCellValue(row.getCell(26)),
                            "holidayMonth", dataFormatter.formatCellValue(row.getCell(23)),
                            "maritalStatus", dataFormatter.formatCellValue(row.getCell(14)),
                            "statusId", "PARTY_ENABLED",
                            "userLogin", userLogin
                    );
                    ...
                    postalAddress.put("address1", dataFormatter.formatCellValue(row.getCell(17)));
                    postalAddress.put("city", dataFormatter.formatCellValue(row.getCell(28)));
                    postalAddress.put("postalCode", "Soguipah");
...
//
                    ...
                    if (fonct!=null){
                        ...
                        Map<String, Object>   emPoFul = UtilMisc.toMap(
                                ...
                                "partyId", partyId,
                                "category", dataFormatter.formatCellValue(row.getCell(4)),
                                "statusCompany", dataFormatter.formatCellValue(row.getCell(27)),
                                "fromDate", now,
                                "userLogin", userLogin
                        );
        ...
        result = ServiceUtil.returnSuccess(UtilProperties.getMessage(resource, "PartyNewPartiesCreated", UtilMisc.toMap("partiesCreated", numero), locale));
        return result;
    }
