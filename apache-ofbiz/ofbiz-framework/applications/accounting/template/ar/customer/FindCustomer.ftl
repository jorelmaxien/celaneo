...
<div id="findEmployee" class="screenlet">
    <div class="screenlet-title-bar">
        <ul>
            <li class="h3">${uiLabelMap.CommonFind} ${uiLabelMap.Customers}</li>
            <#if parameters.hideFields?default("N") == "Y">
                <li><a href="<@ofbizUrl>findCustomers?hideFields=N${paramList}</@ofbizUrl>">${uiLabelMap.CommonShowLookupFields}</a></li>
            <#else>
                <#if partyList??><li><a href="<@ofbizUrl>findCustomers?hideFields=Y${paramList}</@ofbizUrl>">${uiLabelMap.CommonHideFields}</a></li></#if>
                <li><a href="javascript:document.lookupparty.submit();">${uiLabelMap.PartyLookupEmployee}</a></li>
            </#if>
        </ul>
        <br class="clear"/>
    </div>
    <#if parameters.hideFields?default("N") != "Y">
        <div class="screenlet-body">
            <#-- NOTE: this form is setup to allow a search by partial partyId or userLoginId; to change it to go directly to
             ...
             -->
            <form method="post" name="lookupparty" action="<@ofbizUrl>findCustomers</@ofbizUrl>" class="basic-form">
                ...
                <table cellspacing="0">
                    ...
                    <#--<tr>
                        <td class='label'>${uiLabelMap.PartyPartySite}</td>
                        <td>
                            <select name="stateProvinceGeoId">

                                <option value="Diécké">Diécké</option>
                                <option value="Conakry">Conakry</option>
                                <option value="KOLENTEN">Kolenten</option>
                            </select>
                        </td>
                    </tr>
                    ...
                    <tr><td class="label">${uiLabelMap.PartyPartyDepartment}</td>
                        <td>
                            <input type="text" name="department" value="${parameters.department!}"/>
                        </td>
                    </tr>
                    <tr><td class="label">${uiLabelMap.PartyPartyService}</td>
                        <td><input type="text" name="serviceId" value="${parameters.serviceId!}"/></td>
                    </tr>
                    <tr><td class="label">${uiLabelMap.PartyPartyPosition}</td>
                        <td>

                         <span class="field-lookup">
                            <div id="0_lookupId_EditEmplPosition_emplPositionId_auto"></div>
                             <input type="text" name="emplPositionId" size="25" id="0_lookupId_EditEmplPosition_emplPositionId"
                                    data-lookup-ajax-enabled="true" data-lookup-presentation="layer" data-lookup-request-url="LookupEmplPosition"
                                    data-lookup-form-name="EditEmplPosition" data-lookup-optional-target="" data-lookup-width="640" data-lookup-height="500"
                                    data-lookup-position="topleft" data-lookup-modal="true" data-lookup-show-description="true" data-lookup-default-minlength="2"
                                    data-lookup-default-delay="300" data-lookup-args=""
                                    data-lookup-ajax-url="EditEmplPosition_emplPositionId,https://localhost:8443/humanres/control/LookupEmplPosition,ajaxLookup=Y&amp;searchValueFieldName=emplPositionId"
                                    class="ui-autocomplete-input" autocomplete="off">
                        </td>
                    </tr>-->
                    <tr><td><input type="hidden" name="groupName" value="${parameters.groupName!}"/></td></tr>
                    <tr><td><input type="hidden" name="roleTypeId" value="CUSTOMER"/></td></tr>
                    <#if extInfo == "P">
                    ...
                    <tr align="center">
                        <td>&nbsp;</td>
                        <td><input type="submit" value="${uiLabelMap.PartyLookupCustomer}"/>
                            <a href="<@ofbizUrl>findCustomers?roleTypeId=CUSTOMER&amp;hideFields=Y&amp;lookupFlag=Y</@ofbizUrl>" class="smallSubmit">${uiLabelMap.CommonShowAllRecords}</a>
                        </td>
                    </tr>
                </table>
            </form>
...
<#if partyList??>
    ...
    <div id="findEmployeeResults" class="screenlet">
        <div class="screenlet-title-bar">
            <ul>
                <li class="h3">${uiLabelMap.FoundCustomers}</li>
                <#if (partyListSize > 0)>
                    <#if (partyListSize > highIndex)>
                        <li><a class="nav-next" href="<@ofbizUrl>findCustomers?VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex-1}&amp;hideFields=${parameters.hideFields?default("N")}${paramList}</@ofbizUrl>">${uiLabelMap.CommonNext}</a></li>
                    <#else>
                        <li class="disabled">${uiLabelMap.CommonNext}</li>
                    </#if>
                    <li>${lowIndex} - ${highIndex} ${uiLabelMap.CommonOf} ${partyListSize}</li>
                    <#if (viewIndex > 0)>
                        <li><a class="nav-previous" href="<@ofbizUrl>findCustomers?VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex-1}&amp;hideFields=${parameters.hideFields?default("N")}${paramList}</@ofbizUrl>">${uiLabelMap.CommonPrevious}</a></li>
                    <#else>
...
<#--                    <td>${uiLabelMap.PartyRegistrationNumber}</td>-->
...
<#--                        <td>-->
<#--                            <#if partyRow.registrationNumber?has_content>-->
<#--                                <a href="<@ofbizUrl>CustomerProfile?partyId=${partyRow.partyId}</@ofbizUrl>">${partyRow.registrationNumber}</a>-->
<#--                            <#else>-->
<#--                               (PAS DE MATRICULE)-->
<#--                            </#if>-->
<#--                        </td>-->
                        ...
                        <td><#if partyRow.getModelEntity().isField("lastName") && lastName?has_content>
                            ...
                            <#elseif partyRow.getModelEntity().isField("groupName") && partyRow.groupName?has_content>
                                ${partyRow.groupName}
                            <#else>
                                <#assign partyName = Static["org.apache.ofbiz.party.party.PartyHelper"].getPartyName1(partyRow, true)>
...
<#--                        <td><#if partyType.description??>${partyType.get("description", locale)}<#else>???</#if></td>-->
                        <td class="button-col align-float">
                            <a href="<@ofbizUrl>CustomerProfile?partyId=${partyRow.partyId}</@ofbizUrl>">${uiLabelMap.CommonDetails}</a>
                        </td>
                    </tr>
        ...
        <#else>
            <div class="screenlet-body">
                <span class="h3">${uiLabelMap.PartyNoCustomersFound}</span>
            </div>
        </#if>
