...
import org.apache.ofbiz.accounting.services.SendMail;
...
public class PurchaseServices {
    ...
    public static final String SAMPLE_XLS_FILE_PATH = "./spreadsheet/ClientsSoguipah.xlsx";
    ...
    public static Map<String, Object> addListCustomer(DispatchContext dctx, Map<String, Object> context) {
        ...
        try (Workbook workbook = WorkbookFactory.create(new File(SAMPLE_XLS_FILE_PATH))) {
            ...
            System.out.println("Workbook has ************************************" + workbook.getNumberOfSheets() + "**************************************** Sheets : ");
                ...
                System.out.println("********************************rowIterator***********************************************" + rowIterator.hasNext() + " rowIterator");
                while (rowIterator.hasNext()) {
                    Row row = rowIterator.next();
                    String nomPrenom = dataFormatter.formatCellValue(row.getCell(2));
                    int lengt = nomPrenom.indexOf(" ");
                    ...
                    if (lengt == -1) {
                        ...
                        firstName = nomPrenom.substring(0, nomPrenom.indexOf(" "));
                        lastName = nomPrenom.substring(nomPrenom.indexOf(" ")+1);
                    }

                    System.out.println("********************************dataFormatter.formatCellValue(row.getCell(1))***********************************************" + dataFormatter.formatCellValue(row.getCell(1)) + " rowIterator");

                    GenericValue party = delegator.makeValue("Party");
                    party.set("partyId",dataFormatter.formatCellValue(row.getCell(0)));
                    ...
                    person.set("partyId",dataFormatter.formatCellValue(row.getCell(0)));
                    person.set("firstName", firstName);
                    person.set("lastName", lastName);
                    delegator.create(person);

                    GenericValue roleType = delegator.makeValue("PartyRole");
                    roleType.set("roleTypeId", "CUSTOMER");
                    roleType.set("partyId",dataFormatter.formatCellValue(row.getCell(0)));
    ...
    public static Map<String, Object> swiftMailler(DispatchContext dctx, Map<String, Object> context) {

        Delegator delegator = dctx.getDelegator();
        SendMail sendMail = new SendMail();
        String partyId = (String) context.get("partyId");
        String emailAddress = (String) context.get("emailAddress");
        System.out.println("partyId has ************************************" + partyId + "**************************************** partyId : ");
        System.out.println("emailAddress has ************************************" + emailAddress + "**************************************** emailAddress : ");
        ...
        if (!emailAddress.equals("")) {
            try {
                ...
                if (person != null) {
                    sendMail.sendConf(emailAddress, "creation compte client", "mr/mne   " + person.getString("firstName") + " " + person.getString("lastName") + " \n" +
                            "votre compte client a ete creer votre code client est :" + partyId);

                } else {
                    sendMail.sendConf(emailAddress, "creation compte client",
                            "votre compte client a ete creer votre code client est :" + partyId);
            ...
            } catch (GenericEntityException | MessagingException e) {
                Debug.logError(e, module);
                return ServiceUtil.returnError(e.getMessage());
            }
        }
