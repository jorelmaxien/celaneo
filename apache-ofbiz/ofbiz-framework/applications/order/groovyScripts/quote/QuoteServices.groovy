...
def createQuote() {
    ...
    GenericValue party = makeValue('Party', parameters)
    emailAddress = EntityUtil.getFirst(ContactHelper.getContactMech(party, "PRIMARY_EMAIL", "EMAIL_ADDRESS", false))
//    Map serviceResult = run service: 'swiftMailler', with: [
//            partyId: newEntity.quoteId,
//            emailAddress: emailAddress.infoString
//    ]
    ...
    if (emailAddress != null) {
        ...
        try {
            GenericValue person = from("Person")
                    .where("partyId", parameters.partyId).queryOne();

            if (person != null) {
                sendMail.sendConf(emailAddress.infoString, "traitement pre-commande", "mr/mne   " + person.getString("firstName") + " " + person.getString("lastName") + " \n" +
                        "votre pre-comande numero: " + newEntity.quoteId+ " est en cours de traitement....");

            } else {
                sendMail.sendConf(emailAddress.infoString, "traitement pre-commande",
                        "votre pre-comande numero: " + newEntity.quoteId+ " est en cours de traitement....");
